...
GitHub encourages unit, integration, and end-to-end coverage and semantic releases. Its maintenance guide also demonstrates building distribution only in release commits; therefore committing dist on every development branch is an established approach, not a universal requirement. At research time, preserving this project's checked-in-dist approach was the smaller change. Current publication behavior is defined in the [release workflow](../../.github/workflows/ci.yml). Source: [Releasing and maintaining actions](https://docs.github.com/en/actions/how-tos/create-and-publish-actions/release-and-maintain-actions).
...
For this small action, README should own consumer usage and the public contract. The existing docs/development.md should own environment setup, build/test commands, synthetic fixtures, source breakpoints, and act workflow runs. A short CONTRIBUTING.md can link there if external contributions become relevant; duplicating the same setup instructions would create drift.
