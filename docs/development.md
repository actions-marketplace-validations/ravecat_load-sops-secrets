...
SOPS integration, source debugging, and workflow tests share the `create` function in [tests/integration/fixture.ts](../tests/integration/fixture.ts). It generates disposable age identities and encrypted synthetic data in private temporary directories without depending on Vitest. Each caller removes its fixture after use; partial creation is cleaned up by the builder. Integration tests register cleanup with Vitest. The test context contains only the action environment: its directory, bundle path, and run method. Scenarios requiring an incorrect key explicitly call `createKey()` in their bodies; this shared generator returns an age private key as text without creating files. The mocked SOPS process in the fast tests remains separate because it supplies malformed documents and controlled failures without encryption or downloads.
...
If publication succeeds but updating `v<major>` fails, rerunning semantic-release without new releasable changes will not repair the alias. Identify the published full version tag, fetch it from `origin`, and resolve its commit with `git rev-parse 'vX.Y.Z^{commit}'`. Verify the release and force-push that SHA to the matching major tag with `git push origin <release-commit-sha>:refs/tags/vX --force`, replacing the placeholders with the published version, its commit, and its major version.
